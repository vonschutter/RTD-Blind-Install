...
oem_old_crypttab_line="$(cat /etc/crypttab | cut -d " " -f 1-10 )"
oem_new_crypttab_line="$(cat /etc/crypttab | cut -d " " -f 1)  $(cat /etc/crypttab | cut -d " " -f 2) /$(udevadm info $(blkid | grep crypto_LUKS|  cut -d : -f 1) |grep by-uuid | cut -d : -f 2 | head -1):/keyfile luks,keyscript=/lib/cryptsetup/scripts/pa$ >> /etc/crypttab"
